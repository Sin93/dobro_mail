...
class Bot(BotResponse):
    ...
    def create_answer(self):
        ...
        if self.information is not None:
            ...
            if len(self.response_data['stack']) == 0:
                if self.check_history('пожертвовать', 'ACTION', add_key=None):
                    error = False
                    ...
                    if error:
                        self.response_data['messages'].append(self.create_message(self.error_message, 'end'))
                    else:
                        message = 'Для оплаты перейдите по ссылке'
                        self.response_data['messages'].append(self.create_message(message, 'paylink', True))
                        self.response_data['messages'].append(self.create_message('Спасибо за ваше пожертвование!', 'start'))
    ...
    @staticmethod
    def create_message(message, keyboard_type=None, inline=False, payload=None):

        return {
            'message' : message,
            'keyboard': Bot.create_keyboard(keyboard_type, payload),
            'inline': inline
        }
    ...
    def create_keyboard(type, payload):
        ...
        if type == 'paylink':
            keyboard.append(Bot.create_button(type='text', label='Внести пожертвование'))
    ...
    def check_history(self, search, key, add_key=None):
        index = 1
