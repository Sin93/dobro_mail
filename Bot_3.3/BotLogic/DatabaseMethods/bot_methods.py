...
import config
from BotLogic.DatabaseMethods.models import Categories, Projects, History, ImageInfo
...
class Database:
    ...
    def get_logs(user_id, limit):
        data = History.select(History.log).where(History.user_id == user_id).order_by(-History.id).limit(limit)
        return data
...
#     print(project.picture.owner_id)
#     print(project.picture.media_id)
