...
PAYMENT_LINK = 'https://dobro.mail.ru/api/vkchatbot2/create_payment/?api_app=vkchatbot2&api_key=89551b60cb453ad3a0bcc6edc169ef1c6628ff99&amount={amount}&project_id={project_id}&user_id={user_id}'
...
REMIND_DELAY = 86400 #seconds

# Время ограничивающее отправку по разнице текущего времени и времени последней оплаты
PAYMENT_DELAY = 86400 #seconds

# Время через которое обновляется база данных
UPDATE_DATABASE_TIME = 86400 #seconds == 1 day

# Максимальное время жизни сообщения
MAX_MESSAGE_TIME = 345600 # seconds == 4 day
# Время через которое удаляются проекты и сообщения из базы данных
DELETE_PROJECTS_AND_MSG_TIME = 86400 # seconds == 1 day
# размер фотографии для карусели минимальный размер 221, 136
