...
class Bot(BotResponse):
    ...
    def send_answer(self):
        print(f'messages: {self.response_data["messages"]}')
            ...
            if len(self.response_data['stack']) == 0:
                ...
                for key, item in self.response_data['stack'].items():
                    ...
                    if key == 'END':
                        ...
                        user = Database.get_or_create_user(self.user_id)
                        print(f"user_dict: {user.__dict__.get('__data__')}")
                ...
                messages_length = len(self.response_data['messages'])
                if messages_length == 1:
                    ...
                    self.check_history(search='', key='', add_key=None, limit=20)
                    self.response_data['messages'].append(
                        self.create_message(message=self.error_message, keyboard_type='end',
                                            keyboard=self.previous_messages[-1]['keyboard']))
        ...
        if type == 'carousel':
            ...
            for project in data:
                carousel_element = ApiMethodsClass.create_carusel_element(
                    ...
                    description=f'Осталось {project.price} {project.currency}',
                    project_link=project.url,
                    photo_id=f'{project.picture.template_owner_id}_{project.picture.template_media_id}',
