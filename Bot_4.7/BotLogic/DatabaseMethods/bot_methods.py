...
class Database:
    ...
    def delete_inactive_projects():
        projects = Projects.select().where(Projects.available == 0)
        for project in projects:
    ...
    @staticmethod
    def delete_old_history(current_time, delta_time):
        history_logs = History.select().where(History.time + delta_time < current_time)
        for history_log in history_logs:
            history_log.delete_instance()
